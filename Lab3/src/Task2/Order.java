...
public class Order {
	...
	public boolean contains2(Product p) {
		Arrays.sort(items, Comparator.comparing(item -> item.getProduct().getId()));
		int left = 0;
		int right = items.length - 1;
		while (left <= right) {
			int mid = left + (right - left) / 2;
			int comparison = items[mid].getProduct().getId().compareTo(p.getId());
			if (comparison == 0) {
				return true;
...
//		 contain
		...
		// contains() bị lỗi khi Product là p1 và p6 vẫn chưa fix được
		// contains2 chạy tốt
		System.out.println(o1.contains(p4));
		System.out.println(o1.contains(p9));
		System.out.println(o1.contains2(p1));
		System.out.println(o1.contains2(p9));
	}
}
